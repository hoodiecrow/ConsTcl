...
You can call \texttt{reg} with one parameter: \emph{name}. \emph{name} is the string that will eventually become the lookup symbol in the standard library. Following \texttt{reg} are two similar procedures: \texttt{regspecial} and \texttt{regmacro}. The former registers special forms like \texttt{if} and \texttt{define}, and the latter registers macros like \texttt{and} or \texttt{when}. FInally there is \texttt{regvar}, which registers variables.

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf reg, regspecial, regmacro (internal)} \\
name & a Tcl string \\
...
The heart of the Lisp interpreter, \texttt{eval} takes a Lisp expression and processes it according to its form. Symbols to the value they refer to (see page \pageref{variable-reference}), numbers etc to their own value (see page \pageref{constant-literal}), and expressions that are lists to the value that \texttt{eval-form} assigns to them.
...
\textbf{quote} special form

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-quote (public)} \\
expr & an expression \\
env & an environment \\
\textit{Returns:} & a Lisp value \\
\hline
...
\textbf{if} special form

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-if (public)} \\
expr & an expression \\
env & an environment \\
\textit{Returns:} & a Lisp value \\
\hline
...
When expressions are evaluated in sequence\index{sequence}, the order is important for two reasons. If the expressions have any side effects, they happen in the same order of sequence. Also, if expressions are part of a pipeline of calculations, then they need to be processed in the order of that pipeline.



\textbf{begin} special form

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-begin (public)} \\
expr & an expression \\
env & an environment \\
\textit{Returns:} & a Lisp value \\
\hline
...
\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-define (public)} \\
expr & an expression \\
env & an environment \\
\textit{Returns:} & nothing \\
\hline
...
\textbf{set!} special form

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-set! (public)} \\
expr & an expression \\
env & an environment \\
\textit{Returns:} & a Lisp value \\
\hline
...
\textbf{lambda} special form

\noindent\begin{tabular}{ |p{1.9cm} p{8cm}| }
\hline
\rowcolor[HTML]{CCCCCC} \multicolumn{2}{|l|}{\bf special-lambda (public)} \\
expr & an expression \\
env & an environment \\
...
\texttt{expand-macro} takes an expression and an environment as a parameter. First, the operator (\emph{op}) and operands (\emph{args}) are extracted to check if expansion is necessary (the operator \texttt{car}, for historical reasons, stands for the first element of a list, while \texttt{cdr} stands for the rest of the elements after the first in a list). If the operator is the symbol \texttt{define} and the first of the operands is something other than a Pair, then expansion is unnecessary and the procedure returns with a code to break the while loop in \texttt{eval}.
...
\index{environment-has-parent? procedure}


Returns \texttt{\#t} if \emph{env} has a parent environment; otherwise returns \texttt{\#f}.
...
\index{environment-parent procedure}


Returns the parent environment of \emph{env}. It is an error if \emph{env} has no parent.
...
\index{environment-bound-names procedure}


Returns a newly allocated list of the names (symbols) that are bound by \emph{env}. This does not include the names that are bound by the parent environment of \emph{env}.
...
\index{environment-bindings procedure}


Returns a newly allocated list of the bindings of \emph{env}; does not include the bindings of the parent environment. Each element of this list takes one of two forms: \texttt{(name)} indicates that name is bound but unassigned, while \texttt{(name . object)} indicates that \emph{name} is bound, and its value is \emph{object}.
...
\index{environment-bound?}


Returns \texttt{\#t} if \emph{sym} is bound in \emph{env} or one of its ancestor environments; otherwise returns \texttt{\#f}.
...
\index{environment-lookup procedure}


\emph{Sym} must be bound in \emph{env} or one of its ancestor environments. Returns the value to which it is bound.
...
\index{environment-assignable? procedure}


Symbol must be bound in environment or one of its ancestor environments. Returns \texttt{\#t} if the binding may be modified by side effect.
...
\index{environment-assign! procedure}


\emph{Sym} must be bound in \emph{env} or one of its ancestor environments, and must be assignable. Modifies the binding to have \emph{obj} as its value, and returns an unspecified result.
...
\index{system-global-environment variable}


The variable \texttt{system-global-environment} is bound to the environment that's the parent of the \texttt{user-initial-environment}. Primitives and system procedures are bound (and sometimes closed) in this environment.

\subsection{user-initial-environment variable}
\label{user-initial-environment-variable}
\index{user-initial-environment variable}


The variable \texttt{user-initial-environment} is bound to the default environment in which typed expressions are evaluated by the top-level REP loop.

\subsection{nearest-repl/environment procedure}
\label{nearest-repl/environment-procedure}
\index{nearest-repl/environment procedure}


Returns the current REP loop environment (i.e. the current environment of the closest enclosing REP loop). When Scheme first starts up, this is the same as \texttt{user-initial-environment}.
...
Changes the current REP loop environment to \texttt{env}. \texttt{env} can be either an environment or a procedure object. If it's a procedure, the environment in which that procedure was closed is the new environment.
...
\index{make-environment special form}
...
\texttt{(make-environment expression \ldots )}
...
\index{the-environment special form}
...
\index{interpreter-environment? procedure}


Returns \texttt{\#t} if \emph{val} is an interpreter environment; otherwise returns \texttt{\#f}.

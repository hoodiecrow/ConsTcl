...
\texttt{pew} was originally named \texttt{pep} after the sequence parse-eval-print. Now it's named for parse-eval-write. It reads and evals an expression, and writes the result. It's the most common command in the test cases, since it allows me to write code directly in Scheme and to get proper Lisp output.
...
\texttt{typeof?} looks at a value's type and reports if it is the same as the given type. To be certain, it looks at the value in two ways: once assuming that the value is a ConsTcl object, and once assuming that the value is an interpreter\footnote{the Tcl interpreter, not ConsTcl} alias for a ConsTcl object. If one of those affirms the type, the procedure returns \#t. By Scheme convention, predicates\index{predicate} (procedures that return either \texttt{\#t} or \texttt{\#f}) have '?' at the end of their name.
...
As long as the object given to \texttt{write} isn't the empty string, it passes it to \texttt{write-value} and writes a newline.
...
\texttt{teq} tests for type equality, i.e. that the values have the same type.
...
\texttt{veq} tests for value equality, i.e. that the values have the same value.
...
The operators \texttt{+}, \texttt{*}, \texttt{-}, and \texttt{/} stand for the respective mathematical operations.
...
They take a number of operands, but at least one for \texttt{-} and \texttt{/}.
...
\texttt{atan} can be called both as a unary (one argument) function and a binary (two arguments) one.
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{call-with-input-file (public)} \\
str & a string \\
proc & a procedure \\
\textit{Returns:} & a Lisp value \\
\hline
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{call-with-output-file (public)} \\
str & a string \\
proc & a procedure \\
\textit{Returns:} & a Lisp value \\
\hline
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{input-port? (public)} \\
val & a Lisp value \\
\textit{Returns:} & a boolean \\
\hline
\end{tabular}
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{output-port? (public)} \\
val & a Lisp value \\
\textit{Returns:} & a boolean \\
\hline
\end{tabular}
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{current-input-port (public)} \\
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{current-output-port (public)} \\
\textit{Returns:} & a port \\
\hline
\end{tabular}
\begin{lstlisting}
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{with-input-from-file (public)} \\
str & a string \\
thunk & a procedure \\
\textit{Returns:} & nothing \\
\hline
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{with-output-to-file (public)} \\
str & a string \\
thunk & a procedure \\
\textit{Returns:} & nothing \\
\hline
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{open-input-file (public)} \\
filename & a string \\
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{open-output-file (public)} \\
filename & a string \\
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{close-input-port (public)} \\
port & an input port \\
\textit{Returns:} & nothing \\
\hline
\end{tabular}
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{close-output-port (public)} \\
port & an output port \\
\textit{Returns:} & nothing \\
\hline
\end{tabular}
...
\begin{tabular}{ |l l| }
\hline
\multicolumn{2}{|l|}{newline (public)} \\
?port? & an output port \\
\textit{Returns:} & nothing \\
\hline
\end{tabular}
...
During a call to the procedure \texttt{circle-area}, the symbol \texttt{r} is bound to the value 10. But we don't want the binding to go into the global environment, possibly clobbering an earlier definition of \texttt{r}. The solution is to use separate (but linked) environments, making \texttt{r}'s binding a \emph{local variable}W{Local\_variable} in its own environment, which the procedure will be evaluated in. The symbols \texttt{*} and \texttt{pi} will still be available through the local environment's link to the outer global environment. This is all part of \emph{lexical scoping}W{Scope\_(computer\_science)\#Lexical\_scope}.

...
\end{description}
\begin{description}
\item[ sequence] Syntax: \texttt{(begin expression \ldots )}. Process: evaluate all expressions, take value of last (see page \pageref{sequence}).
\end{description}
\begin{description}
\item[ definition] Syntax: \texttt{(define var val)}. Process: take a variable and a value and bind the variable (see page \pageref{definition}) to the value.
\end{description}
\begin{description}
\item[ assignment] Syntax: \texttt{(set! var val)}. Process: take a variable and a value and set the variable (see page \pageref{assignment}) to the value.
\end{description}
\begin{description}
\item[ procedure definition] Syntax: \texttt{(lambda formals body)}. Process: see form (see page \pageref{procedure-definition}).
\end{description}
\begin{description}
\item[ procedure call] Syntax: \texttt{(operator operand \ldots )}. Process: see form (see page \pageref{procedure-call}).
\end{description}
...
A variable\index{variable} is an identifier (symbol) bound to a location in the environment. If an expression consists of an identifier it is evaluated to the value stored in that location. This is handled by the helper procedure \texttt{lookup}. It searches the environment chain for the identifier, and returns the value stored in the location it is bound to. It is an error to do lookup on an unbound symbol.

...
Parsing\footnote{See \texttt{https://en.wikipedia.org/wiki/Parsing}}\index{parsing}, or syntactic analysis, is analyzing a sequence of letters, digits, and other characters, conforming to the rules of \emph{external representation}. The result of parsing is an \emph{expression}\index{expression} in internal form.
...
\textbackslash\ emph{Example: \texttt{r} => 10 (a symbol \texttt{r} is evaluated to 10)}
...
\textbackslash\ emph{Example: \texttt{99} => 99 (a number evaluates to itself)}
...
\textbackslash\ emph{Example: \texttt{(quote r)} => \texttt{r} (quotation makes the symbol evaluate to itself, like a constant)}
...
\textbackslash\ emph{Example: \texttt{(if (> 99 100) (* 2 2) (+ 2 4))} => 6}
...
\textbackslash\ emph{Example: \texttt{(begin (define r 10) (* r r))} => 100}
...
\textbackslash\ emph{Example: \texttt{(define r 10)} => ... (a definition doesn't evaluate to anything)}
...
\textbackslash\ emph{Example: \texttt{(set! r 20)} => 20 (\texttt{r} is a bound symbol, so it's allowed to assign to it)}
...
\textbackslash\ emph{Example: \texttt{(lambda (r) (* r r))} => \texttt{::oo::Obj3601} (it will be a different object each time)}
...
\textbackslash\ emph{Example: \texttt{(+ 1 6)} => 7}
...
A \texttt{Procedure} object is a closure\index{closure}\footnote{See \texttt{https://en.wikipedia.org/wiki/Closure\_(computer\_programming)}}, storing the procedure's parameter list, the body, and the environment that is current when the object is created, i.e. when the procedure is defined.
...
\texttt{write} is implemented in the write\footnote{See page \pageref{output}} section.
